...
_DPL_Init:
		...
		.if(INCR_BUILD = 1)
			...
			PWMDRV_1ch_INIT 2	; PWM1A
			PWMDRV_ComplPairDB_INIT 6
			PWMDRV_ComplPairDB_INIT 7
...
_DPL_ISR:
		...
		CLRC  	AMODE
		CLRC  	PAGE0,OVM
;		CLRC	INTM					; clear interrupt mask - comment if ISR non-nestable  ¿É·ñÇ¶Ì×
;-----------------------------------------------------------------------------------------
...
; call DP library modules
		...
		.if(INCR_BUILD = 1)
			PWMDRV_1ch 1		; PWM1A
			PWMDRV_1ch 2		; PWM1A
			PWMDRV_ComplPairDB 6
			PWMDRV_ComplPairDB 7
...
; Interrupt management before exit
	...
	.if(EPWM7)
		MOVW 	DP,#_EPwm7Regs.ETCLR
		MOV 	@_EPwm7Regs.ETCLR,#0x01			; Clear EPWM7 Int flag
	.endif ; EPWM7
