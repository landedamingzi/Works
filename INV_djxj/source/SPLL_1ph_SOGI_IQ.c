...
void SPLL_1ph_SOGI_IQ_init(int Grid_freq, long DELTA_T, SPLL_1ph_SOGI_IQ *spll_obj)
{
	...
	spll_obj->fn=SPLL_SOGI_Q((float)Grid_freq);
	...
	spll_obj->lpf_coeff.B0_lf=SPLL_SOGI_Q(0.0000625);
	spll_obj->lpf_coeff.B1_lf=SPLL_SOGI_Q(0.0000625);
...
void SPLL_1ph_SOGI_IQ_coeff_update(float delta_T, float wn, SPLL_1ph_SOGI_IQ *spll)
{
	...
	float osgx,osgy,temp,osgK=0.5;
	spll->osg_coeff.osg_k=SPLL_SOGI_Q(osgK);
	osgx=(float)(2.0*osgK*wn*delta_T);
	...
	spll->osg_coeff.osg_qb0=SPLL_SOGI_Q((float)(osgK*osgy)*temp);
